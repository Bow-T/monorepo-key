...
class Hotkey {
  ...
  static const Hotkey defaultClipboard = Hotkey(
    keyCode: 9, // 'V'
  ...
  Map<String, dynamic> toJsonWithPrefix(String prefix) => {
        ...
        '${prefix}Modifiers': modifiers.map((m) => m.json).toList(),
  ...
  factory Hotkey.fromJsonWithPrefix(Map<String, dynamic> j, String prefix, Hotkey defaultHk) {
    final code = j['${prefix}KeyCode'];
    final mods = j['${prefix}Modifiers'];
    if (code is! int || mods is! List) return defaultHk;
    final set = mods
        .map((e) => HotkeyModifier.fromJson(e.toString()))
        .whereType<HotkeyModifier>()
        .toSet();
    final hk = Hotkey(keyCode: code, modifiers: set);
    return hk.isValid ? hk : defaultHk;
  }
  ...
  /// Bản công khai của `_macKeyCode` cho recorder dùng trực tiếp.
  static int? macKeyCode(LogicalKeyboardKey key) => _macKeyCode(key);
